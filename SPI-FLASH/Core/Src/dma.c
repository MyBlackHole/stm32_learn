...
uint8_t SendBuff[SENDBUFF_SIZE + 2];
...
void DMA_Test(void) {
  ...
  SendBuff[SENDBUFF_SIZE] = '\r';
  SendBuff[SENDBUFF_SIZE+1] = '\n';
  ...
  if (HAL_DMA_PollForTransfer(&DMA_Handle, HAL_DMA_FULL_TRANSFER, 0xFFFF) == HAL_OK) {
    printf("\r\nUSART1 DMA TX 测试通过 \r\n");
  } else {
    printf("\r\nUSART1 DMA TX 测试失败 \r\n");
  }
