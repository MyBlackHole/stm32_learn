...
void *dev_spi_init(struct dev_spi *spi, enum en_spi_port port,
                   enum en_spi_mode mode, uint16_t spi_baudrate_prescaller) {
  if (port == DEV_SPI1_GPIOA) {
    ...
    spi->dma_tx_flags = DMA1_FLAG_TC3;
    spi->dma_tx_iqrn = DMA1_Channel3_IRQn;
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_SPI1 |
                               RCC_APB2Periph_AFIO,
                           ENABLE);
  ...
  spi->spi_struct.SPI_BaudRatePrescaler =
      spi_baudrate_prescaller; // SPI_BaudRatePrescaler_2;
...
inline void dev_spi_start(struct dev_spi *spi) { spi->port->ODR &= ~spi->nss; }

inline void dev_spi_stop(struct dev_spi *spi) { spi->port->ODR |= spi->nss; }
...
inline void dev_spi_sendCircular8(struct dev_spi *spi, uint8_t *data,
                                  size_t data_len) {
  /* Configure Tx DMA */
  spi->dma_struct.DMA_MemoryBaseAddr = (uint32_t)(data);
  spi->dma_struct.DMA_BufferSize = data_len;
...
inline void dev_spi_recvCircular8(struct dev_spi *spi, uint8_t *data,
                                  size_t data_len) {
  /* Configure Tx DMA */
  spi->dma_struct.DMA_MemoryBaseAddr = (uint32_t)(data);
  spi->dma_struct.DMA_BufferSize = data_len;
...
inline void dev_spi_sendCircular16(struct dev_spi *spi, uint16_t *data,
                                   size_t data_len) {
...
inline void dev_spi_send16(struct dev_spi *spi, uint16_t *data,
                           size_t data_len) {
  spi->dma_struct.DMA_MemoryBaseAddr = (u32)data;
  spi->dma_struct.DMA_BufferSize = data_len;
...
inline void dev_spi_recv16(struct dev_spi *spi, uint16_t *data,
                           size_t data_len) {
  /* Configure Rx DMA */
  spi->dma_struct.DMA_MemoryBaseAddr = (uint32_t)(data);
  spi->dma_struct.DMA_BufferSize = data_len;
...
inline void dev_spi_recvCircular16(struct dev_spi *spi, uint8_t *data,
                                   size_t data_len) {
  /* Configure Rx DMA */
  spi->dma_struct.DMA_MemoryBaseAddr = (uint32_t)(data);
  spi->dma_struct.DMA_BufferSize = data_len;
